...
| Packet GHCR container runtime (`runtime: container` Action path) | `docs/specs/packet-ghcr-runtime.md`, `docs/packet-workflows.md`, `action.yml`, `docs/specs/publishing-evidence.md` | specified (draft; contract for the reserved container runtime, claim boundary, and verification gate before implementation) | wire the `image` input and `runtime: container` pull path, then verify the gate per release tag |
