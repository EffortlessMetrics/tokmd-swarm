...
| Single-tight ub-review CI gate shape (#226) | `docs/specs/ub-review-ci-gate.md`, `fixtures/ci-gate-contract/reference-ci.yml`, `cargo xtask ci-gate-contract` | specified (phase 3 done, #299: `em-routed-rust-small.yml` retired; routing folded into `ci.yml` `Route CI runner` → `Tokmd Rust Result`) | phase 4: move remaining advisory workflows (cockpit, ripr, coverage upload) to explicit non-required lanes |
