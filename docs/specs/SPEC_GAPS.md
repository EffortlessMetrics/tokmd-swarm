...
| Cross-tool packet preset renderer (`tokmd render`) | `docs/specs/tokmd-packets-render.md`, `crates/tokmd/schemas/tokmd-packets.schema.json`, `fixtures/tokmd-packets/minimal/` | specified (phase 1: schema + minimal renderer) | phase 2: jsonschema CLI validation, sibling bundle ingestion (follow-up from #222 / PR #300) |
