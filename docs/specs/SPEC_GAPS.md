...
| Repo snapshot portability seam (RepoSnapshot / VirtualFile / FileProvider over `ReadFs`) | `docs/specs/repo-snapshot.md`, `crates/tokmd-io-port/src/lib.rs`, `crates/tokmd-scan`, `crates/tokmd-core/tests/archive_host_receipt_parity.rs` | specified (draft; `ReadFs`/`HostFs`/`MemFs` landed; archive/ZIP ingestion sub-seam landed with fail-closed admission, ZIP codec, scan consumer, and lang/module/export host receipt parity; `RepoSnapshot`/`VirtualFile` types still proposed only) | promote explicit `RepoSnapshot`/`VirtualFile` types behind an internal boundary; keep host/in-memory/archive parity oracles current as consumers widen |
| WASM FFI byte mode for browser archive upload | `docs/specs/wasm-ffi-byte-mode.md`, `docs/browser-capability-matrix.md`, `crates/tokmd-core/src/ffi/byte_mode.rs`, `crates/tokmd-wasm/src/lib.rs`, `crates/tokmd-core/tests/archive_zip_ffi_bytemode.rs` | implemented (draft; core `run_json_bytes` + `tokmd-wasm` `runJsonBytes` with lang/module/export byte/JSON envelope parity, fail-closed admission, host receipt parity, and `wasm-bindgen-test` boundary coverage; capability matrix promoted) | wire `runJsonBytes` into the browser runner UI (`web/runner` zipball path); extend analyze byte-mode public-entrypoint parity if/when browser presets widen |
