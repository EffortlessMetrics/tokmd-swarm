...
| Single-tight ub-review CI gate shape (#226) | `docs/specs/ub-review-ci-gate.md`, `fixtures/ci-gate-contract/reference-ci.yml`, `cargo xtask ci-gate-contract`, `policy/ci-lane-whitelist.toml` | specified (phase 4 done: cockpit/ripr/coverage upload are standalone `blocking = false` lanes in `policy/ci-lane-whitelist.toml` — `pr_cockpit_report`, `ripr_advisory`, `rust_coverage` — and only `Tokmd Rust Result` is a required branch-protection context) | keep the single required `Tokmd Rust Result` context and the advisory posture of satellite workflows aligned when lanes change |
