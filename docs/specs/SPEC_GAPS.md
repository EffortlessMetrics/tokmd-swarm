...
| Repo snapshot portability seam (RepoSnapshot / VirtualFile / FileProvider over `ReadFs`) | `docs/specs/repo-snapshot.md`, `crates/tokmd-io-port/src/lib.rs` | specified (draft; FileProvider role exists as `ReadFs`/`HostFs`/`MemFs`, snapshot types proposed only; archive/ZIP ingestion sub-seam with path-safety + zip-bomb limits specified, not implemented) | implement `RepoSnapshot`/`VirtualFile` behind an internal boundary with host/in-memory parity proof; implement `ArchiveProvider` ingestion (fail-closed path/size limits) behind an optional feature |
| WASM FFI byte mode for browser archive upload | `docs/specs/wasm-ffi-byte-mode.md`, `docs/browser-capability-matrix.md`, `crates/tokmd-core/src/ffi/mod.rs`, `crates/tokmd-wasm/src/lib.rs` | specified (draft; transport seam only — admission/scan layers landed per the snapshot spec, but no byte FFI mode or `Uint8Array` binding exists yet, so browser ZIP upload stays experimental) | implement the feature-gated core byte mode + `tokmd-wasm` `Uint8Array` binding with byte/host and byte/JSON parity plus `wasm-bindgen-test` coverage, then promote the archive rows in `docs/browser-capability-matrix.md` / `docs/capabilities/wasm.json` |
