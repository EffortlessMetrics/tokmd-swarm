...
| `1.15.0` (stable candidate tag) | pending candidate proof | pending exact-tag runtime proof | **staged, not yet supported**; do not tag until the candidate gate passes |
...
The `1.15.0` entry is staged by the unreleased stable-prep commit but remains
