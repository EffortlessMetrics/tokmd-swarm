...
Distribution caveat (historical, resolved for publication GHCR):

- the release workflow's Docker/GHCR publish job completed successfully;
- immediately after release, unauthenticated
  `docker manifest inspect ghcr.io/effortlessmetrics/tokmd:1.13.1` returned
  `denied`;
- the available GitHub token could not inspect the package API because it lacked
  `read:packages`.

Maintainer verification on **2026-06-21** records
`ghcr.io/effortlessmetrics/tokmd` as **verified-public** for `v1.13.1`. This
does not invalidate the verified crates.io or GitHub release install paths, and
it was not resolved by rewriting tags or rerunning release mutation. Swarm workbench GHCR (`ghcr.io/effortlessmetrics/tokmd-swarm`) is
workbench/experimental. Maintainer verification on **2026-06-24** records
`:main` as **verified-public** (bootstrap `linux/amd64` only; receipt
`target/publishing/ghcr-visibility-swarm-main.md`; workflow run
https://github.com/EffortlessMetrics/tokmd-swarm/actions/runs/28067135388).
Issue #264 closed for the swarm visibility slice.
