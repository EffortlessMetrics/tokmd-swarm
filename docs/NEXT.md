...
- `cargo xtask doc-artifacts --check` now validates the source-of-truth artifact policy, required docs, active-goal links, doc family status values, ADR filenames, and required sections. It remains an explicit xtask check and is not yet wired into the broader docs gate.
