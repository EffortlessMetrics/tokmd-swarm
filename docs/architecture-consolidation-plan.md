...
| Gate command | `crates/tokmd/src/commands/gate.rs` and `crates/tokmd/src/commands/gate/` | `gate.rs` 396; render owner 111 | Gate policy loading, ratchet loading, and receipt computation remain in the command coordinator while text/JSON result rendering lives in a narrow owner module. Keep `tokmd_gate` proof scoped to both command paths. |
| Sensor command | `crates/tokmd/src/commands/sensor.rs` and `crates/tokmd/src/commands/sensor/` | `sensor.rs` 286; findings owner/tests 344; gate mapping owner/tests 189 | Sensor finding emission and cockpit-evidence-to-envelope gate mapping now live in owner modules while the command keeps cockpit execution and the 3-layer output topology; split rendering or output only when fresh evidence shows the command coordinator is still too broad |
