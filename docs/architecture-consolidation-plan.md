...
| Git adapter boundary | `crates/tokmd-git/src/lib.rs`, `crates/tokmd-git/src/command.rs`, and `crates/tokmd-git/src/intent.rs` | git adapter coordinator 542; command/env isolation owner 82; intent classifier owner 165 | Keep public history, diff, base-ref, and intent APIs stable from `lib.rs`; keep subprocess environment scrubbing and `git_cmd` construction in `command.rs`; keep Conventional Commit and keyword-based intent classification in `intent.rs`. Preserve `git_subprocess_boundary` for subprocess behavior and `tokmd_git_intent_classification` for classifier behavior as git adapter files move. |
