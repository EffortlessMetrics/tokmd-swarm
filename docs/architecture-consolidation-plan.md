...
| Analysis effort implementation | `crates/tokmd-analysis/src/effort/size_basis.rs` and `crates/tokmd-analysis/src/effort/classify.rs` | size-basis aggregation 271; file classification owner 298 | Authored/generated/vendored line aggregation stays in `size_basis.rs`; `.gitattributes` parsing, generated sentinels, path-pattern matching, and heuristic file tagging live in the classification owner. Keep this under the `analysis_effort` proof scope. |
...
| Python binding boundary | `crates/tokmd-python/src/lib.rs`, `crates/tokmd-python/src/args.rs`, and `crates/tokmd-python/src/envelope.rs` | `lib.rs` 1421; argument builder owner 59; envelope helper owner 19 | Keep PyO3 exported wrapper functions and module registration in `lib.rs`; keep shared Python argument-dict construction in `args.rs`; keep envelope extraction and exception mapping in `envelope.rs`. Preserve the `tokmd_python_binding` proof scope. |
...
| Analysis Halstead | `crates/tokmd-analysis/src/halstead/mod.rs`, `halstead/operators.rs`, and `halstead/tokenizer.rs` | report coordinator 201; operator table owner 330; tokenizer owner 116 | Keep aggregate report construction in `mod.rs`, language/operator lookup in `operators.rs`, and per-file token counting in `tokenizer.rs`. Preserve the internal `crate::halstead::*` test surface and keep changes under the `analysis_halstead` proof scope. |
| Analysis near-duplicate detection | `crates/tokmd-analysis/src/near_dup/mod.rs`, `near_dup/clusters.rs`, and `near_dup/fingerprint.rs` | report coordinator 395; cluster owner 305; fingerprint owner 95 | Keep report assembly, eligibility filtering, pairing, and scope partitioning in `mod.rs`; keep cluster construction in `clusters.rs`; keep Winnowing tokenization, k-gram hashing, file reads, and fingerprint selection in `fingerprint.rs`. Preserve the `analysis_near_dup` proof scope. |
