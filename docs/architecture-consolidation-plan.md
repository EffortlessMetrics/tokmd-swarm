...
| Git adapter boundary | `crates/tokmd-git/src/lib.rs` and `crates/tokmd-git/src/command.rs` | git adapter coordinator 650; command/env isolation owner 82 | Keep public history, diff, base-ref, and intent APIs stable from `lib.rs`; keep subprocess environment scrubbing and `git_cmd` construction in `command.rs`. Preserve the `git_subprocess_boundary` proof scope as git adapter files move. |
