...
| CLI parser | `crates/tokmd/src/cli/parser.rs` and `crates/tokmd/src/cli/parser/` | `parser.rs` 956; analyze parser owner 217; context/handoff parser owner 208; cockpit/baseline parser owner 109; diff parser owner 82 | Context, handoff, analyze, cockpit, baseline, and diff argument families now live under parser owner modules; continue command-family splits only when clap snapshots prove behavior is unchanged |
