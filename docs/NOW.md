...
Evidence snapshot: `2026-08-12T12:50:57Z` UTC. Exact heads and the linked PR
...
| --- | --- | --- | --- |
| [#545](https://github.com/EffortlessMetrics/tokmd-swarm/pull/545) | `b82c3c390e077179b410cba70cd53541046f8134` | First-hour inspect → review → evidence → handoff UX; exact-head Tokmd Rust Result, Docs, cockpit, affected/fast, ripr, and policy checks pass; review threads resolved | Merge-ready by source and proof; policy governance is canonically carried by #552; stale external `Codex Review Gate` still blocks merge; no protection mutation |
...
| [#552](https://github.com/EffortlessMetrics/tokmd-swarm/pull/552) | `618bd63bf9e9e5161b51a4a950bea47837a07399` | Canonical in-repo single-maintainer policy: no separate reviewer account/native approval/CODEOWNERS gate; independent agentic review evidence and conversation resolution; exact-head Rust, Droid, ripr, cockpit, Docs, policy, and supporting checks pass; 0 unresolved threads | Does not mutate external GitHub protection; external stale `Codex Review Gate` remains outside repo scope |
| [#560](https://github.com/EffortlessMetrics/tokmd-swarm/pull/560) | `60d3d3e94fb735e3d07df85e7d118030b624114b` | Security report explicitly marks shallow-scan evidence advisory, D-23 unverifiable, and makes no no-regression claim; three current threads resolved; fresh hosted proof is pending/refreshing | Report-only; no security-control or external-service mutation |
...
| [#562](https://github.com/EffortlessMetrics/tokmd-swarm/pull/562) | `a25901663b146b90cbe84ebe11b3fd88868e8fa5` | Advisory agentic UB review decoupled from the deterministic required gate; Rust, contract, UB advisory, ripr, cockpit, and supporting checks pass; Droid provider job was cancelled in its review step after 24m with no diagnostic; 0 unresolved threads | Provider review is unavailable, not source-red; no external protection mutation or merge claim |
...
| [#574](https://github.com/EffortlessMetrics/tokmd-swarm/pull/574) | `76028e192b4e181a24f6b449b300a35178ff7601` | Historical exact-head Tokmd Rust Result, Windows build/test, WASM compile/test, Droid review, PR Plan, and supporting checks passed | PR is closed and conflicting (`DIRTY`); 12 unresolved review threads remain; no merge claim |
| [#595](https://github.com/EffortlessMetrics/tokmd-swarm/pull/595) | `dc8ffe3b13778b04bae45cb4354bed853786eb52` | Positive ratchet-only success test added; review thread `PRRT_kwDOSiTu086YvFS7` replied to and resolved; local targeted rustfmt and `git diff --check` pass; focused Cargo test timed out without diagnostics and remains unverified | No merge claim until fresh hosted proof |
...
The release-status slice [#550](https://github.com/EffortlessMetrics/tokmd-swarm/pull/550) is now at exact head `aa8ca46f85ee62d44c03b93a9b1b9292661d92ff`: the CWD RAII fix and regression coverage are present, and the review thread is resolved; Cargo proof remains unverified. No merge or release claim is made.
