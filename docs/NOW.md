...
Evidence snapshot: `2026-08-12T11:28:10Z` UTC. Exact heads and the linked PR
...
| [#574](https://github.com/EffortlessMetrics/tokmd-swarm/pull/574) | `609b2fd41be6c16865f17dcd8e54186f8e1dff01` | Main-based promotion now also structurally scopes CI contract jobs from the `jobs` mapping with inferred indentation and strengthens the advisory-placement negative test; exact-head required Rust, PR Plan, contract/reference, supporting, and UB checks pass; 2 review findings addressed and resolved; `full-ci` is explicit after the 244 LEM estimate | Local formatting/diff checks pass; Droid advisory and Codecov are pending; targeted Cargo tests timed out without diagnostics on the saturated host; stale external `Codex Review Gate` remains; no publication or merge claim |
