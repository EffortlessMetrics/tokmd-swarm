...
| [#574](https://github.com/EffortlessMetrics/tokmd-swarm/pull/574) | `609b2fd41be6c16865f17dcd8e54186f8e1dff01` | Main-based promotion now also structurally scopes CI contract jobs from the `jobs` mapping with inferred indentation and strengthens the advisory-placement negative test; exact-head contract/reference checks pass; 2 review findings addressed and resolved; `full-ci` selected after PR Plan measured 244 LEM | Local formatting/diff checks pass; targeted Cargo tests timed out without diagnostics on the saturated host; required Rust and Droid checks are pending; PR Plan failed before the explicit `full-ci` label rerun; stale external `Codex Review Gate` remains; no publication or merge claim |
