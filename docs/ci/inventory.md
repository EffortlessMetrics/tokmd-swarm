...
| `tokmd_rust_result` | Tokmd Rust Result | `ci.yml` | mixed | 25 | yes | **Only required context.** Serial `gate --check`, workspace tests, xtask tests, and `proof-policy --check`; advisory ub-review runs independently. |
