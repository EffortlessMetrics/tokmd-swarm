...
| `proof-run-artifacts-check.json` | `tokmd.proof_run_artifacts_check.v1` | `target/proof-run/proof-run-artifacts-check.json` | `cargo xtask proof-run-artifacts-check --json-output <path>` | Verifies the required proof-run summary and records checked counts, guard reason, source path, and validation errors. | It verifies only `proof-run-summary.json`; it does not execute proof or promote advisory evidence. |
...
| `proof-artifacts-check.json` | `tokmd.proof_artifacts_check.v1` | `target/proof/proof-artifacts-check.json` | `cargo xtask proof-artifacts-check --json-output <path>` | Verifies planned, non-executed executor summary/manifest consistency and records source paths, checked counts, guard reason, and validation errors. | It does not prove coverage artifacts exist and does not run advisory proof. |
| `proof-execution-artifacts-check.json` | `tokmd.proof_execution_artifacts_check.v1` | `target/proof/proof-execution-artifacts-check.json` | `cargo xtask proof-execution-artifacts-check --json-output <path>` | Verifies executed executor artifacts, including coverage artifact presence/shape where declared. | It does not promote scoped coverage, upload Codecov, or replace required proof. |
...
| artifact-check text/JSON outputs | verifier-specific | `target/proof/*check*` | `cargo xtask proof-artifacts-check`, `proof-execution-artifacts-check`, or `proof-run-artifacts-check` | Shows whether planned, executed, or required proof artifacts satisfy their local verifier contract. | Prefer the JSON receipt when it exists; text output is for human logs and summaries. |
