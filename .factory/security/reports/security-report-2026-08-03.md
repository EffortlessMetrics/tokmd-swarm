...
**1 commit** on `main`: `70569ba release: prepare v1.15.0-rc.1 candidate` from
2026-08-03 (today). It is a single, GPG-signed release-prep commit that lands
the v1.15.0-rc.1 candidate ("Merge the exact-head reviewed RC preparation and
artifact contract fixes") in this branch. The local working tree is a shallow
clone (`fetch-depth: 1` from the scheduled scan step), so the diff against the
missing parent appears as the full 2,577-file / 1,019,239-line workspace root;
this is the same release-prep surface that `tokmd-swarm` carries into the
publication repository per the documented swarm-routing rules in
`docs/ci/swarm-routing.md`. Every security-critical control surface that the
prior weekly scans (`2026-07-20`, `2026-07-27`) and the comprehensive
true-merge baseline (`2026-06-29`) verified was re-read in-place this scan and
...
- `action.yml` still downloads tokmd with sha256 checksum verification
  ...
  (`1.14.0 1.15.0-rc.1`) using an isolated, anonymous `docker --config` dir
  (defense D-19).
...
regressions introduced by the v1.15.0-rc.1 candidate.
...
Carried observations are unchanged from the `2026-07-27` baseline; the RC1
candidate introduces **no new** low-severity observations this week.
...
Out of scope per `SECURITY.md`. No change in the RC1 candidate.
...
- **Last Modified:** 2026-08-02 (1 day ago — well within 90-day window)
- **Methodology:** STRIDE
- **Next review:** 2026-11-01 (90-day cadence) or upon architecture change
- **No regeneration this scan** — within freshness window and the v1.15.0-rc.1
  candidate introduces no new external surface, subprocess invocation, or
  trust-boundary shift.

### Scan Metadata

- **Commits Scanned:** 1 (`70569ba release: prepare v1.15.0-rc.1 candidate`,
  2026-08-03, GPG-signed)
- **Files in scope:** 2,577 (release-candidate prep cut; shallow clone
  surface). The full surface was previously reviewed under the
  `2026-06-29` true-merge baseline; this scan re-verified all
  security-critical modules in place.
- **Scan Duration:** ~7m (focused diff review + defense re-verification)
...
The coverage below applies to the v1.15.0-rc.1 release-candidate diff and
the re-verified standing defenses.
...
The 7-day window (2026-07-27 → 2026-08-03) contains **1 commit** on `main`:

```
70569bae9b651fcd0499b102c0e4dcf4a091824e
Author: Steven Zimmerman, CPA <15812269+EffortlessSteven@users.noreply.github.com>
Date:   Mon Aug 3 04:36:02 2026 -0400
Subject: release: prepare v1.15.0-rc.1 candidate

    Merge the exact-head reviewed RC preparation and artifact contract fixes.
```

- **Type:** GPG-signed single-commit release prep.
- **Signing:** `gpgsig` is present and well-formed (BLAKE3 + Ed25519-style
  signature block; signature verification is a separate GitHub control-plane
  concern and is not assessed here).
- **Surface:** 2,577 files changed, 1,019,239 lines added. Because the local
  working tree is a shallow clone (`fetch-depth: 1` from the scheduled scan
  step), the diff against the missing parent appears as the full workspace
  root; this is the documented v1.15.0-rc.1 release-prep cut that
  `tokmd-swarm` carries into the publication repository per
...
- **Security-critical files re-read in place:**
  ...
  - `action.yml` — sha256 checksum verification on downloaded tokmd
    ...
    (`1.14.0 1.15.0-rc.1`) for `runtime: container`; isolated anonymous
