...
(`a6b9118 merge(swarm): import GHCR support status docs`) is the swarm
repository initial import, comprising 2470 files. This is a superset of the
content reviewed by the prior weekly scans on 2026-06-01 and 2026-06-08, and
the additional 14 files added in this commit (compared to the 2456 reported
on 2026-06-08) were re-reviewed against the STRIDE threat model. The codebase
continues to demonstrate a security-first design with all standing defenses
(per `.factory/threat-model/threat-model.md`) verified intact. The threat
model (last updated 2026-06-01) remains current and is well within the
...
| D-13 | Content reads bounded by `ContentLimits` (128 KiB default) | `crates/tokmd-analysis/src/content/mod.rs`, `crates/tokmd-analysis/src/content/io/read.rs` | ✓ |
...
| D-21 | Workspace clippy footgun lints (suspicious/nonsensical/ineffective open options, path_buf_push_overwrite, join_absolute_paths, exit) | `Cargo.toml` | ✓ |
| D-22 | Cargo.toml `[patch.crates-io]` for `home` documented in threat model | `Cargo.toml` (out of scope) | ✓ |
| D-23 | `parse_audit_output` tolerates malformed JSON by returning `Pending`, never `Pass` | `crates/tokmd-cockpit/src/supply_chain.rs` | ✓ |
...
- **Commits Scanned:** 1 (`a6b9118 merge(swarm): import GHCR support status docs`)
- **Files in scope:** 2470 (entire repository — initial import of swarm repo)
- **Scan Duration:** ~12m
...
| GitHub workflows | `.github/workflows/*.yml` (22 files), `.github/settings.yml`, `action.yml` | 0 |
...
Subject: merge(swarm): import GHCR support status docs
```

This commit is the initial import of the swarm repository
(`git log --all --oneline` returns exactly 1 commit on `main`). It
introduces 2470 files covering:

- The full Rust workspace source tree (`crates/`, `xtask/`, `fuzz/`)
- All GitHub workflows (`.github/workflows/*.yml`)
- Agent and skill manifests (`.claude/`, `.codex/`, `.github/agents/`,
  `.jules/`, `.factory/`)
- Documentation, plans, and policy documents
- The composite `action.yml`
- The browser-based `web/runner/` JavaScript modules

The prior weekly scans (2026-06-01, 2026-06-08) covered essentially the
same content with 0 findings at the `medium` threshold.

**Review of the change (this scan):**
- Touches 2470 files (initial import). No new security-sensitive surface
...
- `Command::new(...)` invocations across production code all use
  `arg()` with static literal arguments, or pass user-controlled
  values via `arg()` (not `args(&[user_string])` with shell
  metacharacters).
- `Command::new(...)` invocations in test code that use `.args([...])`
  exclusively pass static literal arrays — no shell metacharacter risk.
- Git environment isolation (`GIT_REPO_SHAPING_ENV` removal) is
  consistently applied across `tokmd-git`, `tokmd-scan::walk::git`,
  `tokmd-cockpit`, and `tokmd` (via `git_support.rs`).
- FFI input validation (`validate_in_memory_input_path`) continues to
  reject all known bypass paths.
- Bounded path canonicalization under `ValidatedRoot` is intact.
- Content reads are bounded by `ContentLimits` (128 KiB per file,
  total `max_bytes` cap).
- `unsafe_code = "forbid"` workspace lint remains in force; the only
  `unsafe` blocks in the codebase are in test files
  (`tokmd-cockpit/tests/git_env_boundaries.rs`,
  `tokmd-scan/tests/walk_git_env_boundaries.rs`) for testing env
  variable boundary isolation — necessary and contained.
