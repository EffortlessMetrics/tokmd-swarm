...
(`5e8edc6 merge(swarm): import steward docs manifest fix`) is a true-merge
import (Swarm-Range `79c99739..ab489870`) bringing the steward docs manifest
fix and the full dual-repo state into the swarm branch. The commit adds 2579
files (matching the entire repo), so this scan reviews the same source tree
as the prior scan on 2026-06-29 against a freshly-imported branch state. The
threat model (last updated 2026-06-01) is still current at 42 days old, well
within the 90-day review window. No security-sensitive surface has been
modified since the prior scan.
...
`fetch()` calls to `api.github.com`. These URLs are hard-coded in
`web/runner/ingest.js` (`fetchRepositoryTree` and `fetchFileBytes` at lines
~531 and ~543). The token (when supplied) is stored in `sessionStorage`
(not `localStorage`) via `web/runner/auth.js` and used as a `Bearer` header.
There is no Subresource Integrity pinning or origin allow-listing on the
client-side fetch surface.

**Why not a finding:**
- All sensitive fetches target `api.github.com`, which is HTTPS and well-known.
...
### OBS-006 (carried): True-merge import reconciles dual-repo graph

| Attribute | Value |
|-----------|-------|
| **Severity** | LOW (informational) |
| **STRIDE Category** | Repudiation |
| **File** | `5e8edc6` (this scan's commit) |
| **Status** | Not a finding — design choice |

**Description:** The single commit in the 7-day window is a true-merge import
with `Swarm-Range: 79c99739..ab489870` and `Swarm-Head: ab489870a0a3fd6c6addfd8f71d2f0a7031c2a3b`.
The merge message reads
"merge(swarm): import steward docs manifest fix". This continues the
deliberate dual-repo topology: the swarm repo carries the primary working
history, and the publication repo receives periodic true-merge imports of
validated content. See `docs/ci/swarm-routing.md` for the full shared-history
topology and routing rules.

**Why not a finding:** This is an architectural choice, not a vulnerability.
The prior weekly scans (`security-report-2026-06-01.md`,
`security-report-2026-06-08.md`, `security-report-2026-06-29.md`) covered
the underlying source tree. This commit adds no security-sensitive surface
...
| D-02 | `unwrap_used`, `expect_used`, `panic`, `unreachable` lints denied | `Cargo.toml` (lines 65-71) | ✓ |
...
| D-04 | Git ref validation (`env_base_ref_is_safe` + `--end-of-options`) | `crates/tokmd-git/src/refs.rs` | ✓ (unchanged from prior scan) |
| D-05 | Bounded path canonicalization under root | `crates/tokmd-scan/src/path/bounded_path.rs` | ✓ |
| D-06 | FFI in-memory input path validation (4096-byte cap, no `..`, no control chars, no drive prefix) | `crates/tokmd-core/src/ffi/inputs.rs` | ✓ |
| D-07 | Strict JSON parsing with type validation | `crates/tokmd-core/src/ffi/parse.rs` | ✓ (unchanged from prior scan) |
...
| D-09 | SHA-pinned Droid-related actions; tag-pinned first-party actions | `.github/workflows/droid*.yml` (SHA), others (tag) | ✓ (unchanged from prior scan) |
| D-10 | Branch protection on `main` (CODEOWNERS, 1 approval, CI required) | `.github/settings.yml` | ✓ (unchanged from prior scan) |
...
| D-13 | Content reads bounded by `ContentLimits` (default `max_file_bytes=128 KiB`, total `max_bytes`) | `crates/tokmd-analysis/src/content/mod.rs` | ✓ (unchanged from prior scan) |
| D-14 | PyO3 FFI invariants (no panic, GIL release, error translation) | `crates/tokmd-python/src/lib.rs` | ✓ (unchanged from prior scan) |
| D-15 | WASM uses `MemFs` (no host fs) | `crates/tokmd-wasm/` | ✓ (unchanged from prior scan) |
| D-16 | `web/runner` browser runner uses `textContent` (no `innerHTML`/`eval`/`new Function`/`document.write`) | `web/runner/main.js` | ✓ (verified via grep) |
| D-17 | `web/runner` token stored in `sessionStorage` (not `localStorage`) | `web/runner/auth.js` | ✓ |
| D-18 | `web/runner` worker protocol allowlists modes & presets | `web/runner/messages.js` | ✓ (unchanged from prior scan) |
...
| D-22 | `run_json` top-level JSON must be an object (strict shape check) | `crates/tokmd-core/src/ffi/mod.rs::run_json_inner` | ✓ (unchanged from prior scan) |
| D-23 | Author DAG import via true-merge commits (no force-push of publication history) | repository topology | ✓ |
| D-24 | No `sh -c`/`bash -c` invocations anywhere in production crates | repository-wide grep | ✓ (no matches) |
| D-25 | All `Command::new` invocations use `.arg()` / `.args()` (no shell string concatenation) | `crates/tokmd-git/`, `crates/tokmd/src/git_support.rs`, `crates/tokmd-scan/src/walk/git.rs`, `crates/tokmd-cockpit/src/supply_chain.rs`, `crates/tokmd-cockpit/src/gates/contracts.rs` | ✓ |
...
| **Age** | 42 days |
| **90-day review window** | ✓ Within window |
| **Next scheduled review** | 2026-09-01 or earlier if scope changes |
| **Architectural drift since last review** | None detected |
| **New external surfaces added since last review** | None |
| **New subprocess invocations** | None (only test-only `Command::new("git")` invocations, all under `crates/*/tests/`) |

The threat model at `.factory/threat-model/threat-model.md` remains current and
accurate. No architectural changes were made in this commit that would
invalidate any of the standing defenses listed in the model.
...
- **Last Modified:** 2026-06-01 (42 days ago — well within 90-day window)
- **Methodology:** STRIDE
...
- **Files in scope:** 2579 (entire repository — true-merge initial import)
- **Scan Window:** 2026-07-06 through 2026-07-13 (UTC)
- **Scan Duration:** ~8m
...
Date:   Sat Jul 4 19:24:57 2026 -0400 (2026-07-04 23:24:57 UTC)
Subject: merge(swarm): import steward docs manifest fix
```

This commit is a true-merge import of a steward documentation manifest fix
(`Swarm-Range: 79c99739..ab489870`) from the publication repository
`EffortlessMetrics/tokmd` into the swarm repository. `git log --all --oneline`
returns exactly 1 commit in this branch's history. The commit contains 2579
files, matching the entire repository tree (`git ls-files | wc -l` returns 2579),
indicating this is a continued true-merge of the full dual-repo state.

**Review of the change:**
- Touches 2579 files (true-merge initial import; same content as the prior
  scan on 2026-06-29). The visible message metadata indicates the upstream
  range covers a "steward docs manifest fix" rather than source changes.
...
- `web/runner` browser code uses safe DOM patterns (textContent only) — verified.
- All `action.yml` and workflows reviewed and verified to be consistent with
  prior assessment.
- `.factory/threat-model/threat-model.md` unchanged.
- Workspace lints (`unsafe_code = "forbid"`, `unwrap_used`, `expect_used`,
  `panic`, `unreachable`, `dbg_macro`, `unimplemented`, `todo`) all denied.
- `Command::new` constructions in production code (e.g. `tokmd-git`,
  `tokmd-cockpit`, `tokmd/src/git_support.rs`, `tokmd-scan/src/walk/git.rs`)
  all use `.arg()` style and `GIT_REPO_SHAPING_ENV` isolation. Verified
  by repository-wide grep that no `sh -c` or `bash -c` invocations exist.
...
The next scheduled security scan runs Monday, 2026-07-20 via
