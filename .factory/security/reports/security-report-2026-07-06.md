...
(`5e8edc6 merge(swarm): import steward docs manifest fix`) is a swarm-head
import that pulls a steward docs manifest fix into the working history. This
commit does not introduce any new security-sensitive surface: it touches agent
manifests, doc-artifact policy, friction logs, and review-packet/receipt
schemas that have already been reviewed by the three prior weekly scans
(`security-report-2026-06-01.md`, `security-report-2026-06-08.md`,
`security-report-2026-06-29.md`) without surfacing findings at the `medium`
threshold. The codebase continues to demonstrate a security-first design with
all standing defenses (per `.factory/threat-model/threat-model.md`) verified
intact. The threat model (last reviewed 2026-06-01) remains current and is
well within the 90-day review window.
...
- No DOM injection surfaces observed: all dynamic data is rendered via
  ...
  grep returning no matches in non-test files).
...
| **File** | `5e8edc6` (single-commit branch — 2579 files) |
| **Status** | Not a finding — design choice |

**Description:** The single commit in the 7-day window is the head of a
single-commit branch in `tokmd-swarm`. The merge message reads
"merge(swarm): import steward docs manifest fix" and references a
`Swarm-Head` and `Swarm-Range` that follow the dual-repo shared-history
topology. This is a deliberate topology choice: the swarm repo carries the
primary working history, and the publication repo receives periodic true-merge
imports of validated content. See `docs/ci/swarm-routing.md` for the full
shared-history topology and routing rules.
...
the same exact content. No security-sensitive surface was modified in this
import.
...
| D-08 | Per-family schema versioning (`SCHEMA_VERSION=2`, `COCKPIT_SCHEMA_VERSION=3`, `HANDOFF_SCHEMA_VERSION=5`, `CONTEXT_SCHEMA_VERSION=4`, `CONTEXT_BUNDLE_SCHEMA_VERSION=2`, `ANALYSIS_SCHEMA_VERSION=9`) | `crates/tokmd-types/src/`, `crates/tokmd-analysis-types/src/lib.rs` | ✓ |
...
| D-13 | Content reads bounded by `ContentLimits` (default `DEFAULT_MAX_FILE_BYTES = 128 KiB`) | `crates/tokmd-analysis/src/content/mod.rs` | ✓ |
...
| D-19 | Composite action installs tokmd with checksum verification (sha256) | `action.yml` | ✓ |
...
| D-24 | Container runtime gating to verification-passed tags only | `action.yml` (runtime=container branch) | ✓ |
...
- **Last Modified (file mtime):** 2026-07-06 (today — restored from merge import)
- **Last Reviewed (in-file metadata):** 2026-06-01 (35 days ago — well within 90-day window)
- **Methodology:** STRIDE
...
- **Files in scope:** 2579 (single-commit branch — full swarm-head import)
- **Insertions:** 1,015,819 lines
- **Scan Duration:** ~7m
...
This commit is the head of a single-commit branch in `tokmd-swarm`. The
commit message identifies it as a `merge(swarm)` import carrying a "steward
docs manifest fix" — agent manifests, doc-artifact policy, friction logs,
and review-packet/receipt schemas. The prior three weekly scans
(`security-report-2026-06-01.md`, `security-report-2026-06-08.md`,
`security-report-2026-06-29.md`) reviewed the same exact content with 0
findings at the `medium` threshold.

**File-type breakdown of the import** (sourced from
`git show --name-only 5e8edc6`):

| Extension | Count | Note |
|-----------|-------|------|
| `.rs` | 1345 | Rust source + snapshot tests |
| `.snap` | 491 | insta snapshot fixtures |
| `.md` | 387 | Documentation + changelogs |
| `.json` | 112 | Manifests, schemas, receipts |
| `.toml` | 63 | Cargo + policy + Dependabot |
| `.yml` | 37 | GitHub workflows + agents |
| `.jsonl` | 27 | Receipt streams |
| `.gitkeep` | 19 | Empty-directory markers |
| `.txt` | 16 | Notes + checksums |
| `.yaml` | 9 | Agent configs |
| `.js` | 9 | Web runner |
| `.mjs` | 7 | Web runner tests |
| `.py` | 5 | Python bindings + examples |
| `.dict` | 5 | Fuzz dictionaries |
| `.sh` | 2 | Hooks |
| `.html` | 2 | Web runner views |
| `.ts` | 5 | TypeScript bindings + examples |
| `proptest-regressions` | 8 | Property-test seed corpus |

**Review of the change:**
- Touches 2579 files (single-commit branch import). No new security-sensitive
...
- `web/runner` browser code uses safe DOM patterns (`textContent` only;
  no `innerHTML`/`eval`/`new Function`/`document.write` in non-test files).
